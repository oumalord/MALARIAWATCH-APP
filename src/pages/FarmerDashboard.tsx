...
const CLIMATE_NOTIFICATIONS: ClimateNotification[] = [
  {
    ...
    summary: 'Rainfall is trending above the recent seasonal pattern in your area.', timing: 'Next 7 days', severity: 'Watch',
  ...
  },
  {
    ...
    summary: 'Regional climate agencies are monitoring warmer ocean conditions that may bring heavier or irregular rain.', timing: 'Seasonal outlook', severity: 'Advisory',
  ...
  },
  {
    ...
    summary: 'The forecast shows a possible gap in rainfall. Water availability may become tighter if the pattern continues.', timing: 'Next 2–4 weeks', severity: 'Advisory',
...
export default function FarmerDashboard({ user, onSignOut }: { user: SessionUser; onSignOut: () => void }) {
  ...
  return <main className="min-h-screen bg-[#F6F7F4] pb-8 text-[#14201A]">
    ...
    <div className="mx-auto max-w-6xl px-4 py-5 sm:px-8 sm:py-8"><div className="mb-6"><p className="text-xs font-bold uppercase tracking-[0.08em] text-[#0A5A41]">Farmer dashboard</p><h1 className="mt-1 font-display text-2xl font-bold sm:text-3xl">Good morning, {user.name.split(' ')[0]}</h1><p className="mt-1 text-sm text-[#55665C]">Local conditions and early warnings for your farm.</p></div>
      ...
      <section className="mt-5 rounded-2xl border border-[#E2E6DE] bg-white p-5"><div className="flex flex-col gap-3 sm:flex-row sm:items-start sm:justify-between"><div><p className="text-xs font-bold uppercase tracking-[0.08em] text-[#0A5A41]">Farmer notifications</p><h2 className="mt-1 font-display text-base font-bold">What changed and what to do</h2><p className="mt-1 text-xs text-[#55665C]">Weather and climate updates for {county.name}. Check here whenever you receive a notification.</p></div><button type="button" onClick={enableNotifications} disabled={notificationsEnabled} className="inline-flex w-full items-center justify-center gap-2 rounded-lg border border-[#BFD7C8] px-3 py-2 text-xs font-bold text-[#0A5A41] disabled:cursor-default disabled:bg-[#F3FAF5] sm:w-auto"><Bell size={14} /> {notificationsEnabled ? 'Phone alerts enabled' : 'Enable phone alerts'}</button></div><div className="mt-4 grid gap-3 md:grid-cols-3">{CLIMATE_NOTIFICATIONS.map((notification) => { const Icon = NOTIFICATION_ICONS[notification.kind]; const expanded = expandedNotification === notification.id; return <article key={notification.id} className="rounded-xl border border-[#E2E6DE] bg-[#FBFCFA] p-4"><div className="flex items-start gap-3"><span className="flex h-9 w-9 shrink-0 items-center justify-center rounded-lg bg-[#E6F0F8] text-[#2E6FA7]"><Icon size={17} /></span><div><div className="flex flex-wrap items-center gap-2"><h3 className="text-sm font-bold">{notification.title}</h3><span className={`rounded-full px-2 py-0.5 text-[10px] font-bold uppercase ${notification.severity === 'Warning' ? 'bg-[#FAE1DE] text-[#B23434]' : notification.severity === 'Watch' ? 'bg-[#FBF1DC] text-[#8B6B14]' : 'bg-[#E6F0F8] text-[#2E6FA7]'}`}>{notification.severity}</span></div><p className="mt-1 text-xs leading-5 text-[#55665C]">{notification.summary}</p><p className="mt-2 text-[11px] font-bold text-[#0A5A41]">Expected: {notification.timing}</p></div></div><button type="button" onClick={() => setExpandedNotification(expanded ? null : notification.id)} className="mt-3 text-xs font-bold text-[#0A5A41]">{expanded ? 'Hide next steps' : 'Show next steps'}</button>{expanded && <div className="mt-3 border-t border-[#E2E6DE] pt-3"><p className="flex items-center gap-1.5 text-xs font-bold text-[#0A5A41]"><CheckCircle2 size={14} /> Recommended actions</p><ol className="mt-2 space-y-1.5 text-xs leading-5 text-[#55665C]">{notification.actions.map((action, index) => <li key={action}><span className="mr-1 font-bold text-[#0E7C5A]">{index + 1}.</span>{action}</li>)}</ol></div>}</article>; })}</div></section>
