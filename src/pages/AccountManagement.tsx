...
import { createStaffAccount, getStaffAccounts, setStaffAccountStatus, type StaffAccount } from '../lib/api';
...
export default function AccountManagement({ user, onSignOut }: { user: SessionUser; onSignOut: () => void }) {
  ...
  async function createAccount(event: React.FormEvent<HTMLFormElement>) {
    ...
    try {
      await createStaffAccount({ name: name.trim(), email: email.trim(), role: accountRole as 'admin' | 'enumerator', organisation: canCreateAdmin ? 'MalariaWatch Administration' : 'MalariaWatch Field Team', county: county || undefined, createdBy: user.accountId });
      await refresh(); setName(''); setEmail(''); setCounty(''); setShowForm(false); setMessage(`${accountRole === 'admin' ? 'Admin' : 'Enumerator'} account created in the shared database. Temporary PIN: 1234.`);
    } catch {
  ...
  async function toggle(accountId: string, active: boolean) {
    try {
      await setStaffAccountStatus(accountId, !active);
      await refresh();
    } catch {
