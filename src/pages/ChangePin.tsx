...
import { changeStaffPin } from '../lib/api';
...
export default function ChangePin({ user, onChanged, onSignOut }: { user: SessionUser; onChanged: (user: SessionUser) => void; onSignOut: () => void }) {
  ...
  async function submit(event: React.FormEvent<HTMLFormElement>) {
    ...
    try {
      await changeStaffPin(user.accountId, current, next);
      onChanged({ ...user, mustChangePin: false });
    } catch {
      setError('Your PIN could not be updated. Check the temporary PIN and try again.');
    }
  }
